...
## 4.3(recap)
    마크 업이나 스타일을 최대한 사용할 수 있는 방법을 알아 보았다.
    Btn컴포넌트를 만들어서 prop 인자들을 사용하여서 해보았다.

    인자는 인자로만 사용하여야한다. 
    prop 전달할 때의 이름과 받아서 사용하는 이름이 같아야한다. 

    PropType을 이용하여서 어던 prop를 받는지 검사가 가능하다. 
    하나의 타입을 받아야할때 사용하면 좋다.
    required는 반드시 있어야하는 것이다.
